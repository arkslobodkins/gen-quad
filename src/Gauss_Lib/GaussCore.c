 ...
 * The workhorse.
 **/
int GaussMaster(int n, double alpha, double beta, int conflag, double abscis[], double weight[])
{
#define FALSE 0
#define  TRUE 1
    int k, m;
   ...
   **/
	...
	do {
	   ...
	   **/
		...
		if (below <= k) min= t;		/* ...or bisect to recover */
		else		max= t;
		t= (min + max)/2;
	    };
...
/**/
int QQp(double t, double *poly, double *deriv)    /* not exported, hence static jtdeb static creates error, hence deleted*/
{
    int k;
